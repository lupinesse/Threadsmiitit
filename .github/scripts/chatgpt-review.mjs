...
Focus on: correctness (logic errors, edge cases, null/undefined), single-purpose functions (flag any doing more than one thing), informative naming (flag single-letter variables outside tight map/filter chains), error handling (never a silent/empty catch — use console.warn/error or rethrow with context), test coverage (every new exported function needs a unit test — app tests live in test/*.mjs, CI-script tests live in .github/scripts/test/*.test.mjs). Ignore auto-generated files: script.js, styles.css, docs/*.html. Be direct and specific; cite file and line for every finding.
