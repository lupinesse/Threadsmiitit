...
### Changed
- Code-split the client bundle (#81): `AdminInbox.jsx` (admin-only), `ChatAssistant.jsx`, and the non-default screens `ScreenKalenteri`/`ScreenLisaa`/`ScreenInfo` (`ScreenMiitit`, the default tab shown on first paint, stays a direct import) are now loaded via `React.lazy` + `Suspense`. The route-level screens use a minimal `Ladataan…` loading placeholder, so switching tabs on a slow connection shows something rather than a blank flash; `AdminInbox` and `ChatAssistant` use a `null` fallback instead, since both are closed-by-default overlay sheets — nothing was visible in their place before they load either. Shrinks the main production chunk from 781.73 kB to 689.31 kB. The build still warns above Vite's 500 kB threshold — `@sentry/react` is the largest remaining piece, but deferring it isn't a simple split: `AppErrorBoundary` needs `Sentry.ErrorBoundary` synchronously at the top of the render tree to catch errors from first paint onward, so that's left as explicit future work rather than rushed into this change.
