...
### Changed
- `/api/chat`'s rate-limit backstop (`netlify/functions/lib/rate-limit.mjs`) is now backed by a shared Netlify Blobs store instead of an in-memory `Map`, so the limit holds across every warm function instance and cold start rather than resetting per instance (#77). `isWithinRateLimit` is now async and keyed by client id alone (the hit store is resolved internally, injectable via an options param for tests), mirroring the `eventsStore.mjs` Blobs pattern. Fails open (allows the request, with a logged error) if the Blobs store itself errors, so a storage hiccup in this backstop layer can't take down the primary chat feature it's defending.
- `/api/chat`'s rate-limit bucket key is now resolved via a new `netlify/functions/lib/client-ip.mjs#resolveClientId`, which falls back to the standard `X-Forwarded-For` header (its left-most entry) before giving up and bucketing under the shared `'unknown'` key — previously it went straight from the Netlify connection-IP header to `'unknown'`, so any client whose request lacked that header would rate-limit-collide with every other such client. A `console.warn` now flags each request that does fall back to `'unknown'`, separate from the existing rate-limit-exceeded log, so this becoming common in production would be visible (#78). Every log line in the handler now includes a `requestId` (Netlify's `x-nf-request-id`, or a generated UUID outside Netlify) so a single request's config, rate-limit, and error logs can be correlated in production.
...
### Fixed
- `/api/auth/delete`, Meta's required data-deletion callback, was a no-op: its own comment claimed the app stores no user data server-side, but `eventsStore.mjs` persists a submitter's Threads id/username/avatar/profile URL (`addedBy`) in Netlify Blobs, and the endpoint never touched it. It now verifies Meta's signed `signed_request` payload (`netlify/functions/lib/metaSignedRequest.mjs`, HMAC-SHA256 against `THREADS_CLIENT_SECRET`), extracts `user_id`, and anonymises every matching event via new `anonymizeEventsByUserId` (`eventsStore.mjs`) — `addedBy` is replaced with `{deleted: true}`; event content (title/date/city/etc.) is kept for archival purposes rather than hard-deleted. A request with a missing or invalid signature is rejected (400) rather than silently confirmed. `auth-uninstall.js`'s identical stale "no user data stored" comment was also corrected — deauthorizing isn't itself a deletion request (#91).
