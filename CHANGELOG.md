...
### Changed
- Code-split the client bundle to clear the `npm run build` "chunks larger than 500 kB" warning (#81). `AdminInbox` and `ChatAssistant` are now loaded via `React.lazy`/`Suspense` on first actual use (admin opens the moderation inbox / user opens the chat assistant) instead of being bundled into the initial chunk. `@sentry/react` (~475 KB, the majority of the previous single chunk) is now loaded and initialised after first paint (`requestIdleCallback`, with a `setTimeout` fallback) rather than blocking initial render; `AppErrorBoundary` is now a plain `Component`-based boundary that reports caught errors to a lazily-loaded Sentry instead of wrapping `Sentry.ErrorBoundary` directly, so the error boundary itself no longer forces Sentry into the first-load bundle. Initial chunk drops from 782 KB (247 KB gzip) to 243 KB (74 KB gzip).
